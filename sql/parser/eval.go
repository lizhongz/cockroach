...
var binOps = map[binArgs]binOp{
	...
	binArgs{Plus, dateType, intervalType}: {
		...
		fn: func(left Datum, right Datum) (Datum, error) {
			return DTimestamp{Time: left.(DDate).toTime().Add(right.(DInterval).Duration)}, nil
		},
	},
	binArgs{Plus, intervalType, dateType}: {
		returnType: DummyTimestamp,
		fn: func(left Datum, right Datum) (Datum, error) {
			return DTimestamp{Time: right.(DDate).toTime().Add(left.(DInterval).Duration)}, nil
		},
	},
	...
	binArgs{Minus, dateType, dateType}: {
		returnType: DummyInterval,
		fn: func(left Datum, right Datum) (Datum, error) {
			return DInterval{Duration: left.(DDate).toTime().Sub(right.(DDate).toTime())}, nil
		},
	},
	...
	binArgs{Minus, timestampType, dateType}: {
		returnType: DummyInterval,
		fn: func(left Datum, right Datum) (Datum, error) {
			return DInterval{Duration: left.(DTimestamp).Sub(right.(DDate).toTime())}, nil
		},
	},
	binArgs{Minus, dateType, timestampType}: {
		returnType: DummyInterval,
		fn: func(left Datum, right Datum) (Datum, error) {
			return DInterval{Duration: left.(DDate).toTime().Sub(right.(DTimestamp).Time)}, nil
		},
	},
...
func (ctx EvalContext) makeDDate(t time.Time) (DDate, error) {
	...
	return DDate(secs / driver.SecondsInDay), nil
}
